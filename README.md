...
Member notifications now load saved moderation/report-review updates from the API,
with 25-item pages, individual and all-notification read actions, and an exact
unread navigation count. They refresh every minute while visible, on window focus,
or with Refresh. Loading/error/empty states do not fall back to sample data.
Read state persists in Supabase; no migration is required. New-comment and
new-Tide notifications are not generated by the current database workflow.
...
Stale edits and uncertain writes require an explicit reload/status check before retrying. Previous decisions and feedback remain in moderation history, and reviewers see **Previous review feedback** alongside a resubmitted Trace. Authors read feedback through an owner-authorized endpoint, not staff routes. No database migration is required; members can read saved review notifications on the Notifications page.
...
3. Choose **Save draft** to save privately. A title and slug are required; draft content can be empty. **Preview** displays current form content through the same component as the reading page without saving it.
...
Run `npm test`, `npm run build`, and `npm run verify:tides`. The automated tests simulate API/database responses for creation, publication, return to draft, archiving, member visibility, admin-only reads/writes, duplicate slugs, validation, conflicting edits, safe text previews and logout during a request. The read-only live script checks the configured Supabase public feed, available public details, missing records and anonymous admin restrictions. It creates no lessons. Optional `TIDETRACE_VERIFY_ACCESS_TOKEN` enables read checks with an existing short-lived account session; never commit or share the token.
...
3. **Tides reading - implemented.** Published lessons, direct detail URLs, loading/error/empty states and pagination are connected. See the Tides workflow and verification instructions above.
